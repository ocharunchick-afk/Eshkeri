...
# ------------------------------------------------------------------------------
# 3.5 BUILD ESHKERI.APK (ANDROID APPLICATION)
# ------------------------------------------------------------------------------
if (Test-Path "$ScriptDir\build_apk.py") {
    Write-Step "3.5" "Building Eshkeri.apk (Android Application for Mobile)"
    & $PythonExe "$ScriptDir\build_apk.py"
    if ($LASTEXITCODE -eq 0 -and (Test-Path "$ScriptDir\Eshkeri.apk")) {
        $apkSize = (Get-Item "$ScriptDir\Eshkeri.apk").Length / 1MB
        Write-Success "Eshkeri.apk compiled and signed successfully ($([math]::Round($apkSize, 2)) MB)"
    } else {
        Write-Warning "Eshkeri.apk build skipped or failed."
    }
}
...
with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as z:
    ...
    android_dir = os.path.join(base, 'Android_App')
...
Write-Host "  2. Android Mobile:   " -NoNewline -ForegroundColor Cyan
Write-Host "$ScriptDir\Android_App\Eshkeri.apk ($([math]::Round((Get-Item "$ScriptDir\Android_App\Eshkeri.apk").Length / 1MB, 2)) MB)" -ForegroundColor Yellow
Write-Host "  3. Setup Installer:  " -NoNewline -ForegroundColor Cyan
Write-Host "$TargetSetup ($([math]::Round((Get-Item $TargetSetup).Length / 1MB, 2)) MB)" -ForegroundColor Yellow
Write-Host "  4. Packed Payload:   " -NoNewline -ForegroundColor Cyan
