...
from PyQt5.QtCore import (
    Qt, QPoint, QSize, pyqtSignal, QTimer, QUrl, QRect, QRectF, QSizeF
)
...
class MainWindow(QMainWindow):
        ...
        if os.path.exists(path_to_try):
            try:
                with open(path_to_try, "r", encoding="utf-8") as f:
                    loaded = json.load(f)
                    filtered = [g for g in loaded if g.get("id") in self.DEFAULT_ALLOWED_IDS or g.get("id", "").startswith("custom_")]
                    existing_map = {g.get("id"): g for g in filtered}
                    # Update standard fields (client_id, process, etc.) from default_list
    ...
    def on_spoofer_status_changed(self, is_running, game_name):
        ...
        if is_running:
            ...
            self.lbl_hud_details.setText("✔ Окно игры запущено на рабочем столе! Discord фиксирует активность.\n💡 Если статус не появился сам: Настройки Discord -> 'Зарегистрированные игры' -> 'Добавить её!'")
...
class AddGameDialog(QDialog):
    def __init__(self, parent=None):
        ...
        title = QLabel("➕ ДОБАВЛЕНИЕ ИГРЫ В СПИСОК (DISCORD RPC & ДЕТЕКТ)")
        title.setFont(QFont("Consolas", 12, QFont.Bold))
        title.setStyleSheet(f"color: {CYBER_YELLOW};")
        layout.addWidget(title)

        # Search / Game Title
        layout.addWidget(QLabel("Название игры (начните вводить для мгновенного поиска):"))
        self.in_name = QLineEdit()
        self.in_name.setPlaceholderText("например: Cyberpunk, Minecraft, Genshin, Counter-Strike...")
        ...
        self.lbl_detect_status = QLabel("ℹ База Discord (24 305 игр): начните вводить название")
        ...
        self.in_client_id.setPlaceholderText("например: 1448369915462549616")
    ...
    def _on_name_search(self, text):
        q = text.lower().strip()
        if not q or len(q) < 2 or not self.detectable_games:
            self.suggestions_list.hide()
            self.lbl_detect_status.setText("ℹ База Discord: введите название для автоподбора")
            self.lbl_detect_status.setStyleSheet("color: #718096; font-size: 11px;")
            return
        ...
        if matches:
            for g in matches:
                ...
                pexe = execs[0].replace("\\", "/").split("/")[-1] if execs else f"{name.replace(' ', '')}.exe"
                item = QListWidgetItem(f"★ {name}  (ID: {gid} | exe: {pexe})")
                item.setData(Qt.UserRole, {"name": name, "id": gid, "exe": pexe})
